...
class MockTradingEnvironment:
    ...
    def buy_stock(self, symbol, quantity):
        ...
        df = fetch_stock_data(symbol, '2023-01-01', '2023-12-31')
    ...
    def sell_stock(self, symbol, quantity):
        ...
        df = fetch_stock_data(symbol, '2023-01-01', '2023-12-31')[-100:]
    ...
    def update_portfolio_value(self):
        ...
        for symbol, holdings in self.portfolio.items():
            current_data = fetch_stock_data(symbol, '2023-01-01', '2023-12-31')
    ...
    def calculate_performance_metrics(self):
        ...
        current_portfolio_value = sum(holdings['shares'] * fetch_stock_data(symbol, '2023-01-01', '2023-12-31')['Close'].iloc[-1] for symbol, holdings in self.portfolio.items()) + self.cash
        ...
        daily_returns = [((self.portfolio[symbol]['shares'] * fetch_stock_data(symbol, '2023-01-01', '2023-12-31')['Close'].iloc[-1]) - (self.portfolio[symbol]['shares'] * self.portfolio[symbol]['average_cost'])) / (self.portfolio[symbol]['shares'] * self.portfolio[symbol]['average_cost']) for symbol in self.portfolio]
        sharpe_ratio = (np.mean(daily_returns) - risk_free_rate) / np.std(daily_returns) * np.sqrt(252)  # Annualizing
        ...
        for symbol in self.portfolio.keys():
            ...
            if df['Short_MA'].iloc[-1] > df['Long_MA'].iloc[-1] and df['RSI'].iloc[-1] < 70:
                ...
                if quantity_to_buy > 0:
                    self.buy_stock(symbol, quantity_to_buy)
...
trading_days = 252  # Example trading days in a year
try:
    for day in range(trading_days):
        trading_env.execute_trades()
        trading_env.update_portfolio_value()
        trading_env.calculate_performance_metrics()
