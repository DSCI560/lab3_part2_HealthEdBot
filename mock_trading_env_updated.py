...
class MockTradingEnvironment:
    ...
    def calculate_performance_metrics(self):
        ...
        daily_returns = [fetch_stock_data(symbol, '2023-01-01', '2023-12-31')['Close'].pct_change()*100 for symbol in self.portfolio]
        data['daily_returns'] = data['Close'].pct_change()
        data["daily_returns"].fillna(0, inplace =True)
        print(np.mean(data['daily_returns']))
        ...
        sharpe_ratio = ((np.mean(daily_returns) - risk_free_rate) / np.std(daily_returns)) * np.sqrt(1)  # Annualizing
...
trading_days = 120  # Example trading days in a year
date_str = '2023-10-30'
